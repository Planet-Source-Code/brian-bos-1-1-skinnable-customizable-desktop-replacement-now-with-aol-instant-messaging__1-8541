...
Private Sub imgNetStatus_DblClick()
...
Else
    ...
    frmPSCTicker.Show
End If
If Translucent Then AlphaBlending picPSCButton.hdc, 0, 0, 25, 30, picDesktopCapture.hdc, picPSCButton.Left, 0, 25, 30, TransLevel
End Sub
...
If Index + S < List1.ListCount Then
    If Button = vbLeftButton Then
        If picProgram(Index).Tag <> "d" Then
                ...
                If Translucent Then AlphaBlending picProgram(Index).hdc, 0, 0, 250, 30, picDesktopCapture.hdc, picProgram(Index).Left, 0, 250, 30, TransLevel
...
If transucent Then AlphaBlending picScrollDown.hdc, 0, 0, 20, 30, picDesktopCapture.hdc, picScrollDown.Left + picSeperator(2).Left + 6, 0, 20, 30, TransLevel
End Sub
...
If Translucent Then AlphaBlending picScrollDown.hdc, 0, 0, 20, 30, picDesktopCapture.hdc, picScrollDown.Left + picSeperator(2).Left + 6, 0, 20, 30, TransLevel
...
If Translucent Then AlphaBlending picScrollUp.hdc, 0, 0, 20, 30, picDesktopCapture.hdc, picScrollUp.Left + picSeperator(2).Left + 6, 0, 20, 30, TransLevel
End Sub
    ...
    If i + S > List1.ListCount - 1 Then
        lblTitle(i).Caption = ""
            If picProgram(i).Tag <> "n" Then
                ...
                If Translucent Then AlphaBlending picProgram(i).hdc, 0, 0, 250, 30, picDesktopCapture.hdc, picProgram(i).Left, 0, 250, 30, TransLevel
                picProgram(i).Tag = "n"
            End If
    Else
...
If GetSetting("Bos", "BSystem", "Translucent", "True") = "True" Then
    Translucent = True
Else
    Translucent = False
End If
...
Select Case Screen.Width / Screen.TwipsPerPixelX
    ...
    Case Is < 800
        ...
        End
    Case Else
        MsgBox "Your screen is set to an abnormal resiloution." & vbCrLf & "Please change your resiloution to 1600X1200, 1280x1024, 1024x768, or 800X600." & vbCrLf & "Falling back on Windows Explorer."
        Shell "explorer.exe"
        End
End Select
...
frmLoading.ProgressBar1.Value = frmLoading.ProgressBar1.Value + 3
If Translucent Then
        AlphaBlending Me.hdc, 0, 5, Me.ScaleWidth, Me.ScaleHeight, picDesktopCapture.hdc, 0, 0, Me.ScaleWidth, Me.ScaleHeight, TransLevel
        Blend picBstart, picDesktopCapture, TransLevel, 0, 0, picBstart.Width, picBstart.Height
        AlphaBlending picLocationBar.hdc, 0, 0, picLocationBar.Width, picLocationBar.Height, Me.hdc, 64, 7, picLocationBar.Width, picLocationBar.Height, 255 - TransLevel
        AlphaBlending picSystemTray.hdc, 0, 0, 100, 30, picDesktopCapture.hdc, Me.ScaleWidth - 100, 0, 100, 30, TransLevel
        AlphaBlending picScrollDown.hdc, 0, 0, 20, 30, picDesktopCapture.hdc, picScrollDown.Left + picSeperator(2).Left + 6, 0, 20, 30, TransLevel
        AlphaBlending picScrollUp.hdc, 0, 0, 20, 30, picDesktopCapture.hdc, picScrollUp.Left + picSeperator(2).Left + 6, 0, 20, 30, TransLevel
    For i = 0 To picSeperator.Count - 1
        AlphaBlending picSeperator(i).hdc, 0, 0, 6, 30, picDesktopCapture.hdc, picSeperator(i).Left, 0, 6, 30, TransLevel
    Next
End If
frmLoading.ProgressBar1.Value = frmLoading.ProgressBar1.Value + 10
    For i = 0 To 4
        AlphaBlending Me.hdc, frmBDesktopIcons.ScaleWidth, i, Me.ScaleWidth - frmBDesktopIcons.ScaleWidth, 1, picDesktopCapture.hdc, frmBDesktopIcons.ScaleWidth, i, Me.ScaleWidth - frmBDesktopIcons.ScaleWidth, 1, (5 - i) * 50
    Next
...
Next
If Translucent Then AlphaBlending picPSCButton.hdc, 0, 0, 25, 30, picDesktopCapture.hdc, picPSCButton.Left, 0, 25, 30, TransLevel
frmLoading.ProgressBar1.Value = frmLoading.ProgressBar1.Value + 5
...
frmLoading.ProgressBar1.Value = frmLoading.ProgressBar1.Value + 1
For i = 0 To 4
AlphaBlending Me.hdc, 5, i, frmBDesktopIcons.ScaleWidth - 5, 1, frmBDesktopIcons.hdc, 5, frmBDesktopIcons.ScaleHeight - 5 + i, frmBDesktopIcons.ScaleWidth - 5, 1, (5 - i) * 50
Next
frmLoading.ProgressBar1.Value = frmLoading.ProgressBar1.Value + 1
...
Private Sub PictureBox_SendTo_KeyDown(KeyCode As Integer, Shift As Integer)
   Select Case KeyCode
    ...
    Case vbKeyEnd
        If Shift = 2 Then
            Shutdown = MsgBox("Do you really want to shutdown BoS?", vbYesNo Or vbInformation, "Shutdown BoS?")
